...
dotnet build RobotProject.csproj --runtime linux-arm64 --nologo --no-self-contained --output 'bin/Debug/robot'
if (-Not $?) {
    exit -1;
}
...
if($ipVersion -eq [System.Net.Sockets.AddressFamily]::InterNetworkV6) {
    # IPv6
    scp -6 -r "./bin/Debug/robot/*" "rompi@[${ip}]:'/mnt/csprojects/${workspaceFolderBasename}/'"
}
else {
    # IPv4
    scp -r "./bin/Debug/robot/*" "rompi@${ip}:'/mnt/csprojects/${workspaceFolderBasename}/'"
}
